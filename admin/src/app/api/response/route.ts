...
export async function POST(req: Request) {
  try {
    ...
    const emails = Array.isArray(email) ? email : [email]; // Assuming bulk actions don't heavily rely on single emails unless explicitly provided per ID

    // For simplicity, if we have multiple IDs, we'll iterate through them
    for (let i = 0; i < ids.length; i++) {
      const currentId = ids[i];
      const currentEmail = emails[i] || emails[0]; // Fallback to first if not arrays matching length
      ...
      if (action === 'delete') {
        ...
        await logAdminAction(user.email!, `Deleted Application`, { target_email: currentEmail, id: currentId });
        continue;
      }
      ...
      if (currentEmail) {
        try {
          ...
          console.error(`Email failed to send for ${currentEmail}.`, e);
        }
      }
      ...
      if (process.env.GOOGLE_SHEET_ID) {
        try {
          const { data: appData } = await supabaseAdmin.from('applications').select('*').eq('id', currentId).single();
          if (appData) {
            await appendToSheet(
          ...
          console.error(`Google Sheets append failed for ${currentId}.`, e);
        }
      }

      await logAdminAction(user.email!, `${action === 'accept' ? 'Accepted' : 'Rejected'} User`, { target_email: currentEmail, id: currentId });
    }
